...
colormap={i-256 if i > 127 else i:j for i,j in colormap.items()}
#recmap={colormap[i]:i for i in colormap}
...
def ColourDistance(rgb_1, rgb_2):
     ...
     G = G_1 -G_2
     B = B_1 - B_2
     return ((R**2+G**2+B**2))**3
     

import random
from PIL import ImageSequence
im = Image.open("wang.gif")
imagenum=1
for frame in ImageSequence.Iterator(im):
     frame=frame.resize((384,237))
     for i_ in range(6):
          
          image=frame.crop((   128*(i_%3),128*(i_//3),128*(i_%3)+128,128*(i_//3)+128    ))
          image=image.convert("RGB")
          data=image.getdata()
          image.close()
          print(data[0])
          output=nbtlib.File({"":nbtlib.tag.Compound({"data":nbtlib.tag.Compound({"zCenter":nbtlib.tag.Int(0),"xCenter":nbtlib.tag.Int(0),"scale":nbtlib.tag.Byte(3),'dimension': nbtlib.tag.Int(7)})})})
          colors=[]
          ...
          for i in data:
               ...
               for j,k in colormap.items():
                    ...
                    if new<st:
                         ...
                         nd=new
                         ndi=j
               colors.append(nbtlib.tag.Byte(ndi if random.randrange(st+nd)<st else sti))
          output.root["data"]["colors"]=nbtlib.tag.ByteArray(colors)
          
          output.save(f"output/-{imagenum+i_}.dat", gzipped=True)
     imagenum+=6
